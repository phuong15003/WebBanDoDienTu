  ...
  module Admins
    class ReportsController < Admins::ApplicationController
      ...
      def set_params_date_by_year
        ...
        @date_start_second = (Time.zone.parse("#{params[:year]}") - 1.year).beginning_of_month.utc.strftime('%Y-%m-%d %H:%M:%S')
        @date_end_second = (Time.zone.parse("#{params[:year]}") - 1.year).end_of_month.utc.strftime('%Y-%m-%d %H:%M:%S')

        @date_start_third = (Time.zone.parse("#{params[:year]}") - 2.year).beginning_of_month.utc.strftime('%Y-%m-%d %H:%M:%S')
        @date_end_third = (Time.zone.parse("#{params[:year]}") - 2.year).end_of_month.utc.strftime('%Y-%m-%d %H:%M:%S')
      end
